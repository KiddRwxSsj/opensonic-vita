 ...
 *
 * --- PS Vita port ---
 * fatal_error() used to drop Allegro into text mode and pop a native
 * message box. There's no text mode or window manager here, so it just
 * logs and exits; see logfile.c for where that log ends up.
 */
...
/* Upstream locked this function's code page with END_OF_FUNCTION(game_quit)
 * because video.c registered it via LOCK_FUNCTION()/set_close_button_callback()
 * as a DOS/Allegro real-mode interrupt handler (the OS's window-close signal
 * could fire while the rest of the process was paged out). This port's
 * video.c never registers a close-button callback -- there's no window
 * manager on the Vita to raise that signal -- so there is nothing to lock
 * against, matching timer.c, which drops the equivalent LOCK_VARIABLE/
 * LOCK_FUNCTION pair for the same reason. */
