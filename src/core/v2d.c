...
v2d_t v2d_normalize(v2d_t v)
{
    ...
    /* If v has an Inf/NaN component, m is Inf or NaN too, and v.x/m or
     * v.y/m degenerates to Inf/Inf = NaN (m > EPSILON is still true for
     * m == Inf). That NaN would otherwise flow straight into
     * camera.position in camera_update() and get stuck there forever,
     * since NaN fails every subsequent "> threshold" check. Treat any
     * non-finite input as "no direction" instead of propagating NaN. */
    if(!isfinite(v.x) || !isfinite(v.y) || !isfinite(m))
        return v2d_new(0, 0);

    return (m > EPSILON) ? v2d_new(v.x/m, v.y/m) : v2d_new(0, 0);
}
