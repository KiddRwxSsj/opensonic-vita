 ...
 *
 * --- PS Vita port ---
 * image_t no longer wraps an Allegro BITMAP*. It's a plain software
 * RGBA8888 surface. Everything below the sprite/actor layer still only
 * touches this header, never the pixel format directly, so the rest of
 * the engine is unaffected by this change. See video.c for how the
 * finished backbuffer is handed off to vita2d for presentation.
 */
...
struct image_t {
    uint32 *pixels; /* RGBA8888, byte order R,G,B,A (matches vita2d texture memory) */
    int w, h;
};
