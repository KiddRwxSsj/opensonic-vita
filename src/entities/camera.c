...
void camera_update()
{
    ...
    /* Self-heal: clip() does NOT sanitize NaN (both its comparisons are
     * false for NaN, so it falls through and returns the NaN unchanged).
     * If camera.position (or .dest, which feeds it) ever went non-finite
     * -- from anywhere upstream, not just through v2d_normalize() above
     * -- fall back to the last known-good target instead of latching a
     * broken camera (and therefore broken screen coordinates for every
     * actor) forever. */
    if(!isfinite(camera.position.x) || !isfinite(camera.position.y)) {
        camera.position = isfinite(camera.dest.x) && isfinite(camera.dest.y) ?
            camera.dest : v2d_new(0.0f, 0.0f);
    }
